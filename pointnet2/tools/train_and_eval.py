...
def train_one_epoch(model, train_loader, optimizer, epoch, lr_scheduler, total_it, tb_log, log_f):
    ...
    for it, batch in enumerate(train_loader):
        ...
        log_print(
            "training epoch %d: it=%d/%d, total_it=%d, loss=%.5f, fg_iou=%.3f, lr=%f"
            % (epoch, it, len(train_loader), total_it, loss.item(), iou.item(), cur_lr),
            log_f=log_f,
        )
...
def eval_one_epoch(model, eval_loader, epoch, tb_log=None, log_f=None):
    ...
    log_print(
        "\nEpoch %d: Average IoU (samples=%d): %.6f" % (epoch, iou_list.__len__(), avg_iou),
        log_f=log_f,
    )
    return avg_iou
...
def train_and_eval(model, train_loader, eval_loader, tb_log, ckpt_dir, log_f):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_it = train_one_epoch(
            model, train_loader, optimizer, epoch, lr_scheduler, total_it, tb_log, log_f
        )
