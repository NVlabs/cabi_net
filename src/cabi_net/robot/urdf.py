...
from urdfpy import URDF, Collision, Inertial, Joint, Link, Material, Transmission, Visual
...
class TorchJoint(Joint):
    ...
    def _rotation_matrices(self, angles, axis):
        ...
        sina = torch.sin(angles).to(self.device)
        cosa = torch.cos(angles).to(self.device)
        M = torch.eye(4, device=self.device).repeat((len(angles), 1, 1)).to(self.device)
        M[:, 0, 0] = cosa
        M[:, 1, 1] = cosa
        M[:, 2, 2] = cosa
        M[:, :3, :3] += (
